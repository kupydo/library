...
from kupydo.internal.registry import SecretFieldDetails
...
def test_write_secret_files_with_valid_data(mocker, tmp_path):
	...
	tools.write_secret_files(tmp_path, sfd_list)
	...
	with secret_file.open('rb') as file:
		...
		assert data["identifier"] == "id1"


def test_write_secret_files_with_empty_list(tmp_path):
	tools.write_secret_files(tmp_path, [])
