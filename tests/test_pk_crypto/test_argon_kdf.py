...
import pytest
import base64
from kupydo.internal import pk_crypto as pkc


def test_init_without_salt():
	password = "TestPassword"
	key = pkc.DerivedArgonKey(password, testing=True)
...
def test_init_with_salt():
	...
	key = pkc.DerivedArgonKey(password, salt=salt, testing=True)
...
def test_invalid_salt_input():
	...
	with pytest.raises(ValueError):
		pkc.DerivedArgonKey(password, salt=invalid_salt, testing=True)


def test_consistent_hash_with_same_salt():
	password = "TestPassword"
	salt = base64.b64encode(b"MyConsistentSalt").decode()
	key1 = pkc.DerivedArgonKey(password, salt=salt, testing=True)
	key2 = pkc.DerivedArgonKey(password, salt=salt, testing=True)
...
def test_real_compute_cost():
	start = time.time()
	pkc.DerivedArgonKey("TestPassword")
	end = time.time()
	delta = end - start
	assert delta > 1.5, \
