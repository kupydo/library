...
import pytest
import base64
from kupydo.internal.project_config import ArgonKDF


def test_init_without_salt():
	password = "TestPassword"
	key = ArgonKDF(password, testing=True)
...
def test_init_with_salt():
	...
	key = ArgonKDF(password, salt=salt, testing=True)
...
def test_invalid_salt_input():
	...
	with pytest.raises(ValueError):
		ArgonKDF(password, salt=invalid_salt, testing=True)


def test_consistent_hash_with_same_salt():
	password = "TestPassword"
	salt = base64.b64encode(b"MyConsistentSalt").decode()
	key1 = ArgonKDF(password, salt=salt, testing=True)
	key2 = ArgonKDF(password, salt=salt, testing=True)
...
def test_real_compute_cost():
	start = time.time()
	ArgonKDF("TestPassword")
	end = time.time()
	delta = end - start
	assert delta > 2, \
		"Argon hash compute cost must be larger than 2 seconds."
